                    ...
                    <div class="widget ra_widgetMarginBody">
                        <div class="widget-header ra_bodyHeaderTitleIcon">
                            ...
                            <div class="widget-buttons">
                                <a href="<?=site_url('centers/addFormDistrict/0')?>">
                                    <i class="fa fa-plus blue"></i> New District
                                </a>
                            </div><!--Widget Buttons-->
                        </div><!--Widget Header-->
                    ...
                    ?>                        
                        <div class="widget">
                            <div class="widget-header ra_bodyHeaderTitleIcon">
                                ...
                                <span class="widget-caption">District List Total : <?=$total?> Showing <?=$start?> to <?=$end?></span>
                                <div class="widget-buttons">
                            ...
                            if(is_array($queryList))
                            {
                                foreach($queryList as $row)
                                {
                                    ...
                                    $divisionName = $this->generallib->getDivisionName($row->division_id);
                            ?>
                                        <tr>
                                            ...
                                            <td class="raTableBodyNR">  
                                                <a href="<?=site_url('centers/editDistrictForm/0/'.$row->id)?>" class="btn btn-info btn-xs edit"><i class="fa fa-edit"></i> Edit</a>
                                                <a href="<?=site_url('centers/deleteDistrict/'.$row->id)?>" class="btn btn-danger btn-xs delete" onclick="return confirm('Do you really want to remove this District?')"><i class="fa fa-trash-o"></i> Delete</a>
                                            </td> 
                                        </tr>                                                                                                                                                        
                            <?php        
...
<script>
    ...
    function dopagination(pageNumber)
    {
        ...
        var url = "<?=site_url("centers/districtListPagination")?>";
        var myRandom = parseInt(Math.random()*99999999); // cache buster
        xmlhttp.onreadystatechange=stateChanged;
        xmlhttp.open("GET", url + "/" + escape(pageNumber) + "/"  + escape(limit) + "/" + escape(description) + "/" + escape(divisionID)  + "/" + myRandom, true);
        xmlhttp.send(null);
    }
    ...
    function searchResult()
    {
        ...
        var myRandom = parseInt(Math.random()*99999999); // cache buster
        xmlhttp.onreadystatechange=stateChanged;
        xmlhttp.open("GET", url + "/" + escape(startoffset) + "/"  + escape(limit) + "/" + escape(description) + "/" + escape(divisionID) + "/" + myRandom, true);
        xmlhttp.send(null);
    }
