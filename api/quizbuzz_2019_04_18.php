...
*/
 ...
 class QuizBuzz
 {
            ...
            */  
            ...
            if($numRows == 1)
            {
                ...
                {    
                    ...
                    {
                       $examID = $inputStrArray['examID'];
                       $examLogID = $inputStrArray['examLogID'];
                       ...
                       return $this->submitExamSheet($examLogID, $sheetID, $answer, $isCorect, $isComplete, $userID);
                    }
                    elseif($action == 'RESENT-EXAM-RESULT')
                    {
                        $examLogID = $inputStrArray['examLogID'];
                        return $this->getResentExamSheet($userID, $examLogID);
                    }                    
        ...
        */
        ...
        if($numRowsExamInfo > 0)
        {
            while ($rowExamInfo = mysql_fetch_assoc($resultExamInfo)) 
            {
                ...
                $sqlQuestions = "SELECT questions, correct, is_complete, rounds FROM users_exams WHERE user_id = '$userID' AND exam_id = '$examID' ORDER BY prepare_times DESC LIMIT 1";
        ...
        */ 
        ...
        if($numRowsExamInfo > 0)
        {
            while ($rowExamInfo = mysql_fetch_assoc($resultExamInfo)) 
            {
                ...
                $examName = $rowExamInfo['exam_name'];
                $attempts = $this->GetExamAttemptInfo($userID, $examID);
                $examInfoArray[] = array('examID' => $examID, 'ExamName' => $examName, 'numExams' => $numExams, 'attempt' => $attempts);
            }    
        }
        else
        {
    ...
    */  
    ...
    function getExamSheetBack($userID, $examID)
    {
        $questionSheet = array();
        $campaignID = $this->campaignID;
        $numExams = $this->numExams;
        $attempts = $this->GetExamAttemptInfo($userID, $examID);
        if($attempts < $numExams)
        {
            // Check Pending Exam
            $sqlCheck = "SELECT 
                         id,
                ...
                if($userExamID)
                {
                    ...
                    if($numRowsQustions > 0)
                    {
                        ...
                        while ($rowQustions = mysql_fetch_assoc($resultQustions)) 
                        {
                            ...
                            if($examSheetID)
                            {
                                $questionSheet[] = array('questionsID' => $questionsID, 
                                                         'question' => $question, 
                                                         'questionOptions' => unserialize($questionOptions), 
                                                         'answer' => $answer,
                                                         'isAnswer' => 0);
        ...
        {
            $resultCode = array('code' => "-4", 'message' => 'Exam Quota Finished', 'accessInfo' => 'NULL');
        }
    ...
    */  
    ...
    |--------------------------------------------------------------------------
    */ 
    function submitExamSheet($examLogID, $sheetID, $answer, $isCorect, $isComplete, $userID)
    {
        // UPDATE EXAM SHHET
        $campaignID = $this->campaignID;
        $examResult = array();
        ...
        if($isComplete == 1)
        {
            $sql = "SELECT * FROM users_exams WHERE id = '$examLogID'";
            $result = mysql_query($sql);
            $numRows = mysql_num_rows($result);
            if($numRows > 0)
            {
            ...
            $totalEarnPorints = $this->GetEarnPoints($userID);
            $examSubmitInfo = array('examLogID' => $examLogID,'totalPoints' => $totalEarnPorints, 'examResult' => $examResult);
    ...
    */  
    ...
    function getResentExamSheet($userID, $examLogID)
    {
        $questionSheet = array();
        ...
        $sqlQustions = "SELECT 
                        ...
                        INNER JOIN exam_sheets b
                        ON a.id = b.question_id
                        WHERE b.user_exams_id = '$examLogID' AND b.user_id = '$userID' ORDER BY b.sequence ASC LIMIT 100";
        ...
        if($numRowsQustions > 0)
        {
            while ($rowQustions = mysql_fetch_assoc($resultQustions)) 
            {
                ...
                if($isAnswer == -1)
                $isAnswer = 0;    
                $questionSheet[] = array('examLogID' => $examLogID,
                                        'sheetID' => $sheetID,
                                        'question' => $question, 
                                        'questionOptions' => unserialize($questionOptions), 
                                        'answer' => $answer,
